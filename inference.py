        ...
        for plot in range(25):
            ...
            for index_in, index_tar, _input, target in dataloader:
                ...
                for i in range(forecast_window - 1):
                    ...
                    torch.reshape(prediction, (49, 1, 2))
                    # prediction1 = prediction[:, :, :1]
                    # print(src[i+1:, :, 0])
                    # print(prediction[:, :, :1].squeeze(1))
                    prediction1 = prediction[:, :, :1].squeeze(-1)

                    next_input_model = torch.cat((src[i+1:, :, 0], prediction1[-1][:].unsqueeze(-1)))
