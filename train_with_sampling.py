...
def transformer(dataloader, EPOCH, k, frequency, path_to_save_model, path_to_save_loss, path_to_save_predictions, device):
    ...
    for epoch in range(EPOCH + 1):
        ...
        for index_in, index_tar, _input, target, sensor_number in dataloader:
            ...
            for i in range(len(target)-1):
                ...
                if prob_true_val: # Using true value as next value
                    ...
                    predicted_x = torch.cat((prediction[-1,:,:].unsqueeze(0), positional_encodings_new_val), dim=2)
                    predicted_y = torch.cat((prediction[-1,:,:].unsqueeze(0), positional_encodings_new_val), dim=2)
                    sampled_src_x = torch.cat((sampled_src.detach(), predicted_x.detach()))
                    sampled_src_y = torch.cat((sampled_src.detach(), predicted_y.detach()))
        ...
        if epoch % 10 == 0: # Plot 1-Step Predictions
            ...
            sampled_src_x = scaler.inverse_transform(sampled_src[:,:,0].cpu()) #torch.Size([35, 1, 7])
            sampled_src_y = scaler.inverse_transform(sampled_src[:,:,0].cpu()) 
            src_x = scaler.inverse_transform(src[:,:,0].cpu()) #torch.Size([35, 1, 7])
            src_y = scaler.inverse_transform(src[:,:,0].cpu())
            target_x = scaler.inverse_transform(target[:,:,0].cpu()) #torch.Size([35, 1, 7])
            target_y = scaler.inverse_transform(target[:,:,0].cpu())
            prediction_x = scaler.inverse_transform(prediction[:,:,0].detach().cpu().numpy()) #torch.Size([35, 1, 7])
            prediction_y = scaler.inverse_transform(prediction[:,:,0].detach().cpu().numpy())
            plot_training_3(epoch, path_to_save_predictions, src_x, sampled_src_x, prediction_x, sensor_number, index_in, index_tar)
            plot_training_3(epoch, path_to_save_predictions, src_y, sampled_src_y, prediction_y, sensor_number, index_in, index_tar)
