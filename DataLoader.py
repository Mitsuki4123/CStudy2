...
class SensorDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        _input = torch.tensor(self.df[self.df["reindexed_id"]==idx][["y", "x"]][start : start + self.T].values)
        target = torch.tensor(self.df[self.df["reindexed_id"]==idx][["y", "x"]][start + self.T : start + self.T + self.S].values)
